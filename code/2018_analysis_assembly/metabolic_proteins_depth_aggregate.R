#### code/2018_analysis_assembly/metabolicProteins/metabolic_proteins_depth_aggregate.R ####
# Benjamin D. Peterson
...
list.o.depths <- list.files(path = "dataEdited/2018_analysis_assembly/metabolicProteins/depth",
                            pattern = "depth.tsv",
                            full.names = TRUE)
...
raw.depth.counts <- lapply(list.o.depths,
                           function(fileName) {
                             ...
                             metagenomeID = fileName %>%
                               gsub("dataEdited/2018_analysis_assembly/metabolicProteins/depth/", "", .) %>%
                               gsub("_depth.tsv", "", .)
...
write.csv(mean.df,
          "dataEdited/2018_analysis_assembly/metabolicProteins/depth/metabolicProtein_depth_clean.csv",
          row.names = FALSE)
