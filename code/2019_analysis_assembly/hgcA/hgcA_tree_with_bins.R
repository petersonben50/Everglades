#### code/2019_analysis_assembly/hgcA/hgcA_tree.R ####
# Benjamin D. Peterson
...
#### Read in tree file ####
hgcA.tree <- readRDS("dataEdited/2019_analysis_assembly/hgcA/phylogeny/hgcA_clean_tree.rds")


#### Read in color vector ####
color.vector <- readRDS("dataEdited/2019_analysis_assembly/hgcA/phylogeny/hgcA_clean_tree_color_vector.rds")
...
hgcA2binTax.df <- read.table("/Users/benjaminpeterson/Documents/research/Everglades/dataEdited/2019_binning/binning_initial/binsFinal/hgcA_bin_key.tsv",
                          sep = '\t',
                          header = TRUE) %>%
  select(binID, hgcA) %>%
...
hgcA.tree.plot <- ggtree(hgcA.tree,
  ...
  geom_nodelab(aes(x = branch),
               vjust = -.3,
               size = 2)
pdf("results/2019_analysis_assembly/hgcA_tree_RAxML_rooted_with_bins.pdf",
    height = 60,
    width = 10)
hgcA.tree.plot
dev.off()
...
pdf("results/2019_analysis_assembly/hgcA_tree_RAxML_rooted_with_bins_depth.pdf",
    height = 60,
    width = 10)
...
pdf("results/2019_analysis_assembly/hgcA_tree_RAxML_rooted_with_bins_depthRelative.pdf",
    height = 60,
    width = 10)
gheatmap(hgcA.tree.plot,
